...
<script>
	...
	/*  			            // 우편번호와 주소 정보를 해당 필드에 넣는다.
					...
					if (len > 3){
						alert('주소는 3개까지 추가 가능합니다');
							...
							var html = '<input type="radio" name="location" id="extralocradio' + len + '" class="radio2" value="loc' + len + '"/>' +
							'<label for="extralocradio' + len + '"></label>'+
							'<strong><span class="addedjuso" id="nickname' + len + '"></span><strong>' + 
							'<img src="/img/extraloc.svg" width="20px" height="20px" class="imgLoc"/>' + 
							'<input type="button" id="btnLoc' + len + 'Input" class="radio2" hidden="hidden" onclick="btnExtraLoc();"/>' +
							'<input type="button" id="btnLoc' + len + 'remove" class="radio2" hidden="hidden" onclick="btnExtraRemove' + len + '();"/>' +
							'<label class="btn btn-default float-right" name="labeledit" for="btnLoc' + len + 'remove">삭제</label>' +
							'<label class="btn btn-default float-right" name="labeledit" for="btnLoc' + len + 'Input">수정</label>' +
							'<div class="jusoText" id="inloc' + len + '">' + roadAddr + ' ' + extraRoadAddr + '</div>';
							alert(html);
							...
							document.getElementById('loc' + len).value = roadAddr + ' ' + extraRoadAddr;
	...
	function btnExtraRemove1(){
		...
		loc1.innerHTML = '<input type="button" id="btnLoc1Input" class="radio2" hidden="hidden" onclick="btnExtraLoc();">'+
	...
	function btnExtraRemove2(){
		...
		loc2.innerHTML = '<input type="button" id="btnLoc1Input" class="radio2" hidden="hidden" onclick="btnExtraLoc();>'+
	...
	function btnExtraRemove3(){
		...
		loc3.innerHTML = '<input type="button" id="btnLoc1Input" class="radio2" hidden="hidden" onclick="btnExtraLoc();>'+
	...
	<div class="card card-primary">
		...
		<div class="card-body">
			...
			<div class="form-group">
				...
				<input type="button" value="중복확인" class="btn btn-default float-right" id="btnVerifyId">
				<input type="text" class="form-control form-control-border"
					id="idinput" placeholder="ID" name="id">
			</div>
			<div class="form-group">
				<label for="exampleInputBorderWidth2">패스워드
			...
			<div class="form-group">
				<label for="exampleInputBorderWidth3">패스워드 확인
				<!-- <code>.form-control-border.border-width-2</code> -->
