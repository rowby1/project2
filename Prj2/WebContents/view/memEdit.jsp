...
<style>
	...
	.card.card-primary {width: 500px;
	 ...
	 #homespan, #officespan, #loc1span, #loc2span, #loc3span, #gpsspan {margin: 20px 0px;}
 	 #tooltip {
	  position: relative;
...
/*  		if (pw1.match(regex2)){
	...
	/*  			            // 우편번호와 주소 정보를 해당 필드에 넣는다.
		            ...
		            var html = '<input type="radio" name="location" id="extralocradio1" class="radio2" value="loc1"/>' +
							...
							'<input type="button" id="btnLoc1Input" class="radio2" hidden="hidden" onclick="btnExtraLoc1(this);"/>' +
	...
	/*  			            // 우편번호와 주소 정보를 해당 필드에 넣는다.
		            ...
		            document.getElementById("sample4_roadAddress").value = roadAddr; */
							...
							'<img src="/img/extraloc.svg" width="20px" height="20px" class="imgLoc"/>' + 
							'<input type="button" id="btnLoc2Input" class="radio2" hidden="hidden" onclick="btnExtraLoc2(this);"/>' +
							...
							var id = 'loc2span';
							document.getElementById(id).innerHTML = html;
							window.open('/view/jusoname2.jsp', 'loc2', 'height=260,width=500,top=100,left=100');
	...
	/*  			            // 우편번호와 주소 정보를 해당 필드에 넣는다.
		            ...
		            document.getElementById("sample4_roadAddress").value = roadAddr; */
							...
							'<img src="/img/extraloc.svg" width="20px" height="20px" class="imgLoc"/>' + 
							'<input type="button" id="btnLoc3Input" class="radio2" hidden="hidden" onclick="btnExtraLoc3(this);"/>' +
							...
							var id = 'loc3span';
							document.getElementById(id).innerHTML = html;
							window.open('/view/jusoname3.jsp', 'loc3', 'height=260,width=500,top=100,left=100');
	...
	function btnHomeRemove(){
		if (document.getElementById('homeradio').checked)
	...
	function btnOfficeRemove(){
		if (document.getElementById('officeradio').checked)
			document.getElementById('current').checked = true;
	...
	function btnExtraRemove1(){
		if (document.getElementById('extralocradio1').checked)
			document.getElementById('current').checked = true;
	...
	function btnExtraRemove2(){
		if (document.getElementById('extralocradio2').checked)
			document.getElementById('current').checked = true;
		var loc2 = document.getElementById('loc2span');
		loc2.innerHTML = '<input type="button" id="btnLoc2Input" class="radio2" hidden="hidden" onclick="btnExtraLoc2();">'+
	...
	function btnExtraRemove3(){
		if (document.getElementById('extralocradio3').checked)
			document.getElementById('current').checked = true;
		var loc3 = document.getElementById('loc3span');
		loc3.innerHTML = '<input type="button" id="btnLoc3Input" class="radio2" hidden="hidden" onclick="btnExtraLoc3();">'+
						   '<label for="btnLoc3Input">장소3 추가하기<img src="/img/extraloc.svg" width="20px" height="20px" class="imgLoc"/></label>';
						   
	}
	
	function loc1getnick(date){
	...
	window.onload = function(){
		...
		// 저장된 주소정보---------------------------------------------------------
		switch (${locstatus}){
		case 0: document.getElementById('current').checked = "true"; break;
		case 1: document.getElementById('homeradio').checked = "true"; break;
		case 2: document.getElementById('officeradio').checked = "true"; break;
		case 3: document.getElementById('extralocradio1').checked = "true"; break;
		case 4: document.getElementById('extralocradio2').checked = "true"; break;
		case 5: document.getElementById('extralocradio3').checked = "true"; break;
		}
		// 아아디 중복체크 ----------------------------------------------------------		
...
<body>
	...
	<div class="card card-primary">
		...
		<div class="card-body">
			...
			<h4 style="display: inline;" id="tooltip">주소 설정</h4>
  			<span id="tooltipbox"><span id="tooltiptext">체크된 주소에 맞춤 정보를 제공합니다</span></span>
			<div class="radio">
				<span id="gpsspan">
				<input type="radio" name="location" id="current" class="radio2" value="current">
				...
				<label for="home">우리집 추가하기<img src="/img/home.svg" width="20px" height="20px" class="imgLoc"></label>
				</div>
				</c:when>
			    ...
			    <input type="button" id="officeremove" class="radio2" hidden="hidden" onclick="btnOfficeRemove();"/>
				<strong>회사</strong>
				<img src="/img/company_loc.svg" width="20px" height="20px" class="imgLoc"/>
				<label class="btn btn-default float-right" for="officeremove">삭제</label>
				<label class="btn btn-default float-right" for="office">수정</label>
			...
			<c:choose>
				...
				<c:when test="${home != null}">
				<input type="text" name="homeloc" 	id="homeloc" value="${home.address }">
				</c:when>
			</c:choose>
			<c:choose>
				<c:when test="${office == null}">
				...
				<c:when test="${office != null}">
				<input type="text" name="officeloc" id="officeloc" value="${office.address }">
				</c:when>
			</c:choose>
			...
			<c:choose>
				...
				<c:when test="${len == 1 }">
				<input type="text" name="loc1name" 	id="loc1name" value="${extraloc[0].name }">
				<input type="text" name="loc1" 		id="loc1" value="${extraloc[0].address }">
				<input type="text" name="loc2name" 	id="loc2name">
				<input type="text" name="loc2" 		id="loc2">
				<input type="text" name="loc3name" 	id="loc3name">
				<input type="text" name="loc3" 		id="loc3">
				...
				<input type="text" name="loc1name" 	id="loc1name" value="${extraloc[0].name }">
				<input type="text" name="loc1" 		id="loc1" value="${extraloc[0].address }">
				<input type="text" name="loc2name" 	id="loc2name" value="${extraloc[1].name }">
				<input type="text" name="loc2" 		id="loc2" value="${extraloc[1].address }">
				<input type="text" name="loc3name" 	id="loc3name">
				<input type="text" name="loc3" 		id="loc3">
				</c:when>
				<c:when test="${len == 3 }">
				...
				<input type="text" name="loc2name" 	id="loc2name" value="${extraloc[1].name }">
				<input type="text" name="loc2" 		id="loc2" value="${extraloc[1].address }">
				<input type="text" name="loc3name" 	id="loc3name" value="${extraloc[2].name }">
				<input type="text" name="loc3" 		id="loc3" value="${extraloc[2].address }">
				</c:when>
			</c:choose>
			</div>
