...
<script>
  window.onload = function() {
	...
	form.addEventListener('submit', function(e) {
		...
		/* case "btnCommentNotify":   // view로 이동
			e.preventDefault(); // submit 취소
			e.stopPropagation();
			
			location.href = "/mboard?cmd=COMMENTNOTIFY" ;
			break; */
...
<body>
 ...
 <%@ include file="/include/chat.jsp" %>
   ...
   <main>
	...
	<form action="/mboard?cmd=COMMENTDELETE&cnum=<%=scnum %>&cid=<%=cid %>" method="POST">
	
	<table id='upcomment'>
	 <caption><h2>댓글 관리</h2></caption>
	 <tr>
	   <td>내용</td>
	   <td><input type="textarea" name="ctext" value="<%=ctext %>" readonly/></td>
	 </tr>
	 <tr>
	   <td colspan="2">
	   <button id="btnCommentDelete" >삭제</button>
<!-- 	   <button id="btnCommentNotify" >신고</button>	   
